...
Current capabilities are read-only public/private repository status, open issue and pull request listings, recent workflow run listings, and GitHub App authentication. The developer verified Milestone 5A's Lambda, Secrets Manager, and private repository access in `eu-central-1`. Milestone 5B prepares a remote HTTP API exposing only `GET /health`; deployment and remote smoke checks remain manual. Remote MCP/repository access, Alexa+, issue creation or editing, pull request editing, workflow reruns, AI analysis, and write actions are not implemented.
...
See [AWS development instructions](docs/aws-development.md) for exact PowerShell identity checks, deployment, positive/negative remote smoke tests, and log discovery. Use IAM Identity Center or an assumed role with temporary credentials; routine deployment as root is prohibited and the documented identity check stops on `:root`. The existing environment already has bootstrap and secret setup; those steps need not be repeated for 5B.

`pnpm infra:synth` is offline; `infra:diff`, `infra:bootstrap`, and `infra:deploy` are explicit manual AWS commands. One HTTP API routes only `GET /health` to the existing Lambda. No `/mcp`, repository, or catch-all route is exposed. Seven-day access logs and 10 requests/second, burst 20 throttling bound development usage without creating a hard spending cap. The runtime role and imported secret remain narrowly scoped. No 5B deployment was performed by Codex. [Alexa authentication research](docs/alexa-auth-research.md) records requirements and Cognito limitations; compatibility is not claimed.
