...
The browser calls same-origin `/api/demo/chat` through a single uncached CloudFront behavior. The REST API has no wildcard CORS. The existing HTTP API retains its two local development CORS origins for health/MCP. No browser build-time API URL or AWS credentials are needed. Both API stages throttle at roughly 10 requests/second with burst 20. The chat Lambda reserves at most two concurrent executions (no provisioned concurrency) to bound simultaneous Bedrock calls. Throttling and concurrency limits are cost controls, not authentication.
