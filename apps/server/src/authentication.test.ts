...
it.each(["direct", "aws"])(
  "uses %s credentials for App discovery and installation reads, reusing the token cache",
  async (mode) => {
    ...
    const secretSend = vi
      ...
      .mockImplementation(async () => ({
        SecretString: JSON.stringify({ appId: "123", privateKey }),
        $metadata: {},
      }));
    const github =
      mode === "aws"
        ? createConfiguredGitHubClient(
            { VOXOPS_GITHUB_SECRET_ID: "voxops/dev/github-app" },
            process.cwd(),
          )
        : createGitHubRepositoryClient({ appId: "123", privateKey });
    ...
    expect(secretSend).toHaveBeenCalledTimes(mode === "aws" ? 1 : 0);
  },
);
